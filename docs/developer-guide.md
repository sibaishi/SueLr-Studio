...
3. engine executor traverses nodes; with one `逐项运行` node, it executes upstream once and then sequentially replays the downstream subgraph once per non-empty item
