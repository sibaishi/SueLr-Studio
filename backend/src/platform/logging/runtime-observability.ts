...
function logProcessLifecycle(event: string, fields: RuntimeEventFields = {}): void {
  logger.info(event, {
    processInstanceId,
    pid: process.pid,
    ppid: process.ppid,
...
export function recordRuntimeEvent(event: string, fields: RuntimeEventFields = {}): void {
  logProcessLifecycle(event, fields);
}

export function installRuntimeObservability(): string {
