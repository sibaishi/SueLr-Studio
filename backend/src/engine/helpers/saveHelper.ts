...
type ContentType = keyof typeof TYPE_DIRS;
type MimeType = keyof typeof MIME_EXT;
...
function targetPath(type: ContentType, options: SaveOptions = {}, ext = 'txt'): string {
...
function detectUrlType(value: string): ContentType {
...
async function saveString(value: string, options: SaveOptions): Promise<SavedFile> {
  if (value.startsWith('data:')) {
    const match = value.match(/^data:([^;]+);base64,(.+)$/);
    if (match) {
      const type = detectUrlType(value);
      const ext =
        MIME_EXT[match[1] as MimeType] ||
        (type === 'image' ? 'png' : type === 'video' ? 'mp4' : type === 'audio' ? 'mp3' : 'bin');
...
export async function saveContentByType(content: unknown, options: SaveOptions = {}): Promise<SavedFile[]> {
