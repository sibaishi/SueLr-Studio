...
async function buildMessageContent(text: string, images: DynamicValue[], scope: RuntimeApiConfig['scope']) {
...
function stringifyPromptForSearch(content: DynamicValue) {
  if (typeof content === 'string') return content.trim();
  if (!Array.isArray(content)) return '';
  return content.filter((part) => part?.type === 'text' && part.text).map((part) => String(part.text).trim()).filter(Boolean).join('\n');
}
...
function buildSearchPlan(text: DynamicValue) {
  ...
  if (/(今天|今日|当天)/.test(normalized)) {
    ...
    return { query: normalized.replace(/今天|今日|当天/g, dateText), topic: isNewsIntent ? 'news' : undefined, days: 1, dateHint: dateText };
  }
  if (/昨天/.test(normalized)) {
    const dateText = buildAbsoluteDateText(-1);
    return { query: normalized.replace(/昨天/g, dateText), topic: isNewsIntent ? 'news' : undefined, days: 2, dateHint: dateText };
  }
  if (/(最新|最近|刚刚|目前|当前|现在)/.test(normalized)) {
    const dateText = buildAbsoluteDateText(0);
    return { query: `${normalized} ${dateText}`, topic: isNewsIntent ? 'news' : undefined, days: isNewsIntent ? 3 : 7, dateHint: dateText };
  }
...
export async function execute(
  ...
  let prompt = '';
  const imageReferences: DynamicValue[] = [];
  ...
  for (let i = 0; i < rawValues.length; i++) {
    ...
    if (type === 'text') {
      ...
      imageReferences.push(value as DynamicValue);
    }
  }
  ...
  if (!apiKey) throw new Error('未配置 API Key，请先在设置页或 API Key 节点中填写。');

  const userPrompt = normalizeTextInput(prompt);
  if (!userPrompt && imageReferences.length === 0) {
