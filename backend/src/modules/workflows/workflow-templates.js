...
export const BUILTIN_WORKFLOW_TEMPLATES = [
  {
    ...
    nodes: [
      ...
      node('preview', 'output', 900, 170),
    ],
    edges: [
      edge('prompt-to-generate', 'prompt', 'text', 'generate', 'prompt'),
      edge('generate-to-preview', 'generate', 'images', 'preview', 'content'),
    ],
    settings: {},
  ...
  },
  {
    ...
    nodes: [
      node('question', 'textInput', 80, 120, {
        text: '请用简洁的中文，帮我把这个想法整理成三个可执行步骤。',
      }),
      node('chat', 'aiChat', 480, 70, {
        model: '',
      ...
      node('answer', 'output', 900, 160),
    ],
    edges: [
      edge('question-to-chat', 'question', 'text', 'chat', 'prompt'),
      edge('chat-to-answer', 'chat', 'response', 'answer', 'content'),
    ],
    settings: {},
    metadata: { builtinTemplate: true },
  },
  {
    ...
    nodes: [
      node('reference', 'imageInput', 80, 60, {
        fileUrl: '',
      }),
      node('prompt', 'textInput', 80, 380, {
      ...
      node('preview', 'output', 940, 250),
    ],
    edges: [
      edge('reference-to-generate', 'reference', 'image', 'generate', 'reference'),
      edge('prompt-to-generate', 'prompt', 'text', 'generate', 'prompt'),
      edge('generate-to-preview', 'generate', 'images', 'preview', 'content'),
    ],
    settings: {},
