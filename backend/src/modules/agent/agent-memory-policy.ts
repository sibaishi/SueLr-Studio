...
export function normalizeMemoryContent(value: DynamicValue, maxLength = 5000): string {
  return cleanString(value, maxLength).replace(/\s+/g, ' ');
}

export function isMalformedMemoryContent(content: DynamicValue): boolean {
...
export function isUnsafeMemoryWriteContent(content: DynamicValue): boolean {
  const text = normalizeMemoryContent(content, 12000);
  if (isMalformedMemoryContent(text)) return true;
  return UNSAFE_MEMORY_PATTERNS.some((pattern) => pattern.test(text));
}

export function normalizeMemoryFingerprint(content: DynamicValue): string {
  const text = normalizeMemoryContent(content, 12000)
    .toLowerCase()
    .replace(/[，。！？、；：,.!?\s]/g, '');
  if (!text) return '';
...
export function normalizeMemoryTags(tags: DynamicValue, maxTags = 8): string[] {
...
export function normalizeMemoryImportance(value: DynamicValue): number {
