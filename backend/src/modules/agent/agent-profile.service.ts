...
function normalizeToolList(value: DynamicValue): string[] {
  if (!Array.isArray(value)) return [];
  return value.map((item) => TOOL_NAME_ALIASES[cleanString(item, 80)] || cleanString(item, 80)).filter(Boolean);
}

function normalizeProfile(profile: DynamicValue): PlainObject | null {
...
function mapLegacyRoles(settings: DynamicValue): PlainObject[] {
  const roles = Array.isArray(settings?.ui?.customRoles) ? settings.ui.customRoles : [];
  return roles
    .map((role: DynamicValue) =>
      normalizeProfile({
        id: role.id,
        name: role.name,
        icon: role.icon,
...
export class AgentProfileService {
  ...
  resolveProfile(profileId: string, model?: string) {
    ...
    const resolved =
      (profileId
        ? profiles.find(
            (profile): profile is PlainObject => Boolean(profile) && (profile as PlainObject).id === profileId,
          )
        : fallback) || fallback;
    const normalizedProfile = resolved as PlainObject;
    return {
      ...normalizedProfile,
      defaultModel: normalizedProfile.defaultModel || model || fallback.defaultModel || '',
    };
  }
}
