...
function profileAllowsMemoryWrite(profile: DynamicValue): boolean {
...
function extractTextContent(content: DynamicValue): string {
...
function buildExtractionText(messages: DynamicValue[], assistantMessage: DynamicValue): string {
...
function parseExtractedMemories(reply: DynamicValue): string[] {
  try {
...
function normalizeCandidateMemory(item: DynamicValue): string {
...
export class AgentMemoryStrategy {
  ...
  constructor({ capabilitiesService, memoryService }: PlainObject) {
    this.capabilitiesService = capabilitiesService;
    this.memoryService = memoryService;
  }

  isEnabled(profile: DynamicValue) {
