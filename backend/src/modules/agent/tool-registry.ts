...
type ToolDefinition = {
  ...
  handler: (args: PlainObject, ctx: PlainObject) => Promise<DynamicValue> | DynamicValue;
};
...
function normalizeToolName(name: DynamicValue): string {
  const clean = cleanString(name, 80);
  return TOOL_NAME_ALIASES[clean] || clean;
}

function jsonOrText(value: DynamicValue): string {
...
function schemaObject(properties: PlainObject, required: string[] = []): PlainObject {
  return {
    type: 'object',
    properties,
    required,
...
function normalizeImageToolResult(data: DynamicValue) {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return data;
  if (Array.isArray(data.artifacts)) return data;
  const images = Array.isArray(data.images)
    ? data.images.filter((item: DynamicValue) => typeof item === 'string' && item)
    : [];
  ...
  return {
    ...
    tool: 'generate_image',
    ...safeData,
    artifacts: images.map((url: string, index: number) => ({
      type: 'image',
      url,
...
function normalizeVideoToolResult(data: DynamicValue, request: PlainObject = {}) {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return data;
  if (Array.isArray(data.artifacts)) return data;
  const videos = []
    .concat(data.video || [])
...
function normalizePositiveInteger(value: DynamicValue, fallback: number, min = 1, max = 20): number {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.min(max, Math.max(min, Math.floor(parsed)));
}

function normalizeVideoDuration(value: DynamicValue, fallback = -1): number {
...
function normalizeImageQuality(value: DynamicValue): string {
...
function normalizeImageRatio(value: DynamicValue): string {
...
function normalizeImageOutputFormat(value: DynamicValue): string {
  const normalized = cleanString(value, 40).toLowerCase();
...
function normalizeVideoRatio(value: DynamicValue): string {
  return normalizeImageRatio(value);
}

function normalizeVideoResolution(value: DynamicValue): string {
  const normalized = cleanString(value, 40).toLowerCase();
  const allowed = new Set(['480p', '720p', '1080p', '2k', '4k']);
  return allowed.has(normalized) ? normalized : '';
}

function isLocalOrInlineImageRef(value: DynamicValue): boolean {
...
function resolveImageRefsForToolArgs(args: PlainObject, ctx: PlainObject): DynamicValue[] {
...
async function resolveImageInputForTool(value: DynamicValue): Promise<string> {
  const str = String(value || '').trim();
  if (!str) return '';
...
async function resolveMediaInputForTool(value: DynamicValue): Promise<string> {
  return resolveImageInputForTool(value);
}

function buildCurrentTimeResult(timezoneInput: DynamicValue) {
  const timezone =
...
function normalizeGroundingText(value: DynamicValue): string {
  return cleanString(value, 12000).toLowerCase().replace(/\s+/g, ' ');
}

function isGroundedInCurrentRequest(value: DynamicValue, currentUserText: DynamicValue): boolean {
  const needle = normalizeGroundingText(value);
  if (!needle) return false;
  return normalizeGroundingText(currentUserText).includes(needle);
}

function collectWorkflowInputStrings(inputs: DynamicValue): string[] {
...
function assertWorkflowExecutionGrounded(args: PlainObject, ctx: PlainObject) {
...
function hasTool(profile: DynamicValue, toolName: string): boolean {
  ...
  return (
    ...
    profile.enabledTools.some((item: DynamicValue) => normalizeToolName(item) === normalizedName)
  );
}
...
export class ToolRegistry {
  ...
  constructor({ capabilitiesService, memoryService, executionService }: PlainObject) {
    ...
    this.tools = [
      {
        ...
        handler: async (args: PlainObject, ctx: PlainObject) => {
      ...
      },
      {
        ...
        handler: async (args: PlainObject) => {
          const query = cleanString(args.query, 4000);
      ...
      },
      {
        ...
        handler: async (args: PlainObject, ctx: PlainObject) => {
          ...
          try {
            ...
            if (normalizedError?.code === 'IMAGE_MODEL_AMBIGUOUS') {
              return jsonOrText({
                type: 'tool_needs_clarification',
                reason: 'multiple_image_models_available',
                message: normalizedError.message,
                operation: normalizedError.details?.operation,
                candidates: normalizedError.details?.candidates || [],
              });
            }
            if (normalizedError?.code === 'IMAGE_MODEL_UNAVAILABLE') {
              return jsonOrText({
  ...
  listTools(profile: DynamicValue, options: PlainObject = {}) {
    return this.tools
      .filter((tool: ToolDefinition) => hasTool(profile, tool.name))
  ...
  toModelTools(profile: DynamicValue, options: PlainObject = {}) {
  ...
  async execute(name: DynamicValue, args: PlainObject, ctx: PlainObject = {}) {
...
export function createToolRegistry(deps: PlainObject) {
