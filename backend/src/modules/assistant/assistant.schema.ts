...
function ensureObject(value: DynamicValue, message: string): PlainObject {
...
function validateId(value: DynamicValue, fieldName: string) {
...
function validateUrlOrEmpty(value: DynamicValue, fieldName: string) {
...
function validateUrlArray(value: DynamicValue, fieldName: string) {
...
function validateDataUrlImage(value: DynamicValue, fieldName: string) {
...
function validateDataUrlVideo(value: DynamicValue, fieldName: string) {
...
function normalizeConversationMessages(record: PlainObject) {
...
function normalizeConversationTimestamp(record: PlainObject) {
  const ts = Number(record.ts ?? record.updatedAt);
  return Number.isFinite(ts) && ts > 0 ? ts : Date.now();
}

function normalizeRelativePath(value: DynamicValue, fieldName: string) {
...
export function validateConversationList(payload: DynamicValue) {
...
export function validateGalleryItem(payload: DynamicValue) {
...
export function validateVideoItem(payload: DynamicValue) {
...
export function validateAssistantFilePath(value: DynamicValue) {
  return normalizeRelativePath(value, 'filePath');
}

export function validateAssistantRecordId(value: DynamicValue, fieldName: string) {
