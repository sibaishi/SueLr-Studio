...
function cleanText(value: DynamicValue, maxLength = 4000) {
  return String(value || '')
    .trim()
    .slice(0, maxLength);
}

function round2(value: DynamicValue) {
...
function writeState(next: DynamicValue) {
...
function extractSessionCookie(response: DynamicValue) {
  const setCookie = response.headers.getSetCookie?.() || [];
  const rawCookie =
    setCookie.find((item: string) => /^session=/i.test(item)) || response.headers.get('set-cookie') || '';
...
function normalizeUser(data: DynamicValue) {
...
function normalizeBalance(data: DynamicValue) {
...
function normalizePositiveInteger(value: DynamicValue, fallback: number, max = 1000) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed < 1) return fallback;
  return Math.min(max, Math.round(parsed));
}

function normalizeLogItem(item: DynamicValue = {}) {
...
function normalizeLogResponse(data: DynamicValue, page: number, pageSize: number) {
  const payload = data?.data;
...
function buildLogQuery(query: PlainObject = {}) {
...
export class AccountDetailsService {
  ...
  async saveCredentials(payload: PlainObject = {}) {
  ...
  async refreshBalance(options: PlainObject = {}): Promise<DynamicValue> {
    ...
    try {
      ...
      if (
        options.retried ||
        !['ACCOUNT_DETAILS_BALANCE_FAILED', 'ACCOUNT_DETAILS_BALANCE_INVALID'].includes((error as DynamicValue)?.code)
      ) {
        throw error;
      }
  ...
  async ensureSession(state: DynamicValue = readState()) {
  ...
  async fetchAndStoreBalance(state: DynamicValue) {
  ...
  async getLogs(query: PlainObject = {}, options: PlainObject = {}): Promise<DynamicValue> {
    const current = options.state || readState();
    ...
    try {
      ...
      if (options.retried || (error as DynamicValue)?.code !== 'ACCOUNT_DETAILS_LOGS_FAILED') {
        throw error;
      }
      const relogged = await this.login({ username: current.username, password: current.password });
