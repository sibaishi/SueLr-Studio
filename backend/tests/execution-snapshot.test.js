...
test('execution snapshot keeps persisted workflow identity and draft source', async () => {
  const { createExecutionSnapshot, CURRENT_SNAPSHOT_VERSION } = await import(`../src/modules/execution/execution-snapshot.js?test=${Date.now()}`);
...
test('execution snapshot generates persisted source when no draft exists', async () => {
  const { createExecutionSnapshot } = await import(`../src/modules/execution/execution-snapshot.js?test=${Date.now()}`);
