...
async function createTestServer(name, env = {}) {
  ...
  const { createApp } = await import(`../src/app/create-app.js?scope=${Date.now()}-${Math.random()}`);
  const app = createApp();
...
test('request scope defaults preserve single-user runtime behavior', async () => {
  const { createRequestScope, summarizeScopeFoundation } = await import('../src/platform/runtime/request-scope.js');
...
test('key service interfaces accept scope-aware options without changing single-user results', async () => {
  ...
  const { WorkflowsService } = await import('../src/modules/workflows/workflows.service.js');
  const { FilesService } = await import('../src/modules/files/files.service.js');
  const { ExecutionService } = await import('../src/modules/execution/execution.service.js');
...
test('broader service interfaces accept scope-aware options without changing single-user results', async () => {
  const scope = { userId: 'single-user', workspaceId: 'default', runtimeMode: 'local-web' };
  const { AssistantService } = await import('../src/modules/assistant/assistant.service.js');
...
test('new persisted resources carry ownership metadata and old records get fallback ownership', async () => {
  ...
  const { WorkflowsService } = await import('../src/modules/workflows/workflows.service.js');
  const { AgentMemoryService } = await import('../src/modules/agent/agent-memory.service.js');
  const { AgentSessionStore } = await import('../src/modules/agent/agent-session-store.js');
  const { AssistantService } = await import('../src/modules/assistant/assistant.service.js');
  const { FilesService } = await import('../src/modules/files/files.service.js');
...
test('scoped storage namespace preserves current single-user layout and prepares workspace paths', async () => {
  const { getStoragePaths } = await import('../src/platform/storage/storage-paths.js');
  const {
    createStorageNamespace,
    getScopedStoragePaths,
    isDefaultStorageScope,
  } = await import('../src/platform/storage/scoped-storage.js');
...
test('scoped output file resolution uses request scope while default scope keeps legacy layout', async () => {
  ...
  try {
    const { getStoragePaths } = await import(`../src/platform/storage/storage-paths.js?static=${Date.now()}`);
    const { getScopedStoragePaths, ensureScopedStorageDirectories } = await import(`../src/platform/storage/scoped-storage.js?static=${Date.now()}`);
