...
test('workflow run logger writes stable run metadata and lifecycle events', async () => {
  ...
  const { createWorkflowRunLogger } = await import(`../src/platform/logging/workflow-run-logger.ts?test=${Date.now()}`);
...
test('workflow run logger can store auxiliary inline payload files', async () => {
  ...
  const { createWorkflowRunLogger } = await import(`../src/platform/logging/workflow-run-logger.ts?artifact=${Date.now()}`);
