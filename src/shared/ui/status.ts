...
export const logLevelColor = (level: string, colors: Colors) =>
  level === 'success' ? colors.green : level === 'error' ? colors.red : level === 'warn' ? colors.orange : level === 'debug' ? colors.text3 : colors.blue;
...
export const taskStatusColor = (status: string, colors: Colors) => {
  ...
  if (['done', 'completed', 'complete', 'success', 'succeeded', 'finished', '已完成'].includes(status)) return colors.green;
