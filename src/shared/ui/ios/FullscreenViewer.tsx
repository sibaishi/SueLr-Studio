    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 99999, background: lightOverlay(T), backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
      {mediaType === 'video' ? (
        <video
          ...
          style={{ maxWidth: '95vw', maxHeight: '95vh', borderRadius: 18, boxShadow: '0 24px 56px rgba(15,23,42,0.24)', background: '#000' }}
        />
      ) : (
      ...
      {onAction && actionLabel && (
        <button
          ...
          style={{ position: 'absolute', left: 20, bottom: 20, minHeight: 40, padding: '0 14px', borderRadius: 20, ...glass(0.15), border: `1px solid ${T.border}`, color: T.text, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 13, fontWeight: 700 }}
        >
          <ImagePlus size={16} />
          {actionLabel}
        </button>
