...
export function ImagePanel({ base, apiKey, models, addLog, bridgeRef, onAddToChat, providerConfig, imageStreamingMode: _imageStreamingMode }: { base: string; apiKey: string; models: ModelInfo[]; addLog: (l: string, m: string) => void; bridgeRef: React.MutableRefObject<BridgeRef>; onAddToChat: (urls: string[]) => void; providerConfig?: ProviderConfig; imageStreamingMode: 'stream' | 'non-stream' }) {
  const T = useT();
