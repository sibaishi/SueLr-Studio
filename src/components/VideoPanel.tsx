...
export function VideoPanel({ base, apiKey, models, addLog, bridgeRef, onAddToChat, providerConfig, videoStreamingMode }: { base: string; apiKey: string; models: ModelInfo[]; addLog: (l: string, m: string) => void; bridgeRef: React.MutableRefObject<BridgeRef>; onAddToChat: (prompt: string, videoUrl?: string) => void; providerConfig?: ProviderConfig; videoStreamingMode: 'stream' | 'non-stream' }) {
  const T = useT();
