...
export function TaskDetailModal({ task, type, onClose, onApply }: {
  ...
  const sizeLabel = t.width && t.height
    ? `${t.width} x ${t.height}${t.sizeSource === 'dimensions' ? '（已优先使用）' : ''}`
    : t.ratio !== 'auto'
      ? `${t.ratio}（按比例）`
      : '自动';
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 99990, background: lightOverlay(T), backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{
        ...
        border: `1px solid ${T.border}`, boxShadow: '0 20px 48px rgba(15,23,42,0.18), 0 4px 12px rgba(15,23,42,0.10)',
      }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {isImage && (
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              <div><div style={{ fontSize: 11, color: T.text3, marginBottom: 4 }}>宽高</div><div style={{ fontSize: 13, color: T.text }}>{sizeLabel}</div></div>
              <div><div style={{ fontSize: 11, color: T.text3, marginBottom: 4 }}>质量</div><div style={{ fontSize: 13, color: T.text }}>{t.quality || '默认'}</div></div>
              <div><div style={{ fontSize: 11, color: T.text3, marginBottom: 4 }}>张数</div><div style={{ fontSize: 13, color: T.text }}>{t.n || 1}</div></div>
              <div><div style={{ fontSize: 11, color: T.text3, marginBottom: 4 }}>格式</div><div style={{ fontSize: 13, color: T.text }}>{t.output_format || 'png'}</div></div>
            </div>
          )}
          <div style={{ display: 'flex', gap: 16 }}>
