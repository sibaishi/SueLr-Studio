...
import { checkSettingsServer, getBackendStatus, loadStorageSettings, pickStorageDirectory, resetStorageSettings, restartBackendRequest, saveStorageSettings, useSettingsPanelController, waitForBackendReady } from '@/features/settings';
import type { SettingsPanelProps } from '@/features/settings';
...
export function SettingsPanel({
  ...
  const saveStoragePathAction = async () => {
    ...
    try {
      ...
      setStorageSettings(next);
      setStoragePathDraft(next.customRoot || '');
      addLog('success', '外部路径已保存，重启后端后生效');
    } catch (error) {
      addLog('error', error instanceof Error ? error.message : String(error));
    } finally {
  ...
  const resetStoragePathAction = async () => {
    ...
    try {
      ...
      setStorageSettings(next);
      setStoragePathDraft('');
      addLog('success', '外部路径已恢复默认，重启后端后生效');
    } catch (error) {
      addLog('error', error instanceof Error ? error.message : String(error));
    } finally {
      setStorageSettingsSaving(false);
  ...
  const restartBackendAction = async () => {
    ...
    try {
      ...
      addLog('success', '后端已重启完成');
      toast('后端已重启完成', 'success');
    } catch (error) {
