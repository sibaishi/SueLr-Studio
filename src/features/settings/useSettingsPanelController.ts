...
export function useSettingsPanelController({
  ...
  const updateActiveConfig = useCallback((patch: Partial<ApiConfig>) => {
    if (!activeConfigId) return;
    setApiConfigs((prev) =>
      prev.map((config) => (config.id === activeConfigId ? { ...config, ...patch } : config)),
    );
  }, [activeConfigId, setApiConfigs]);

  const updateProviderConfig = useCallback((patch: Partial<ProviderConfig>) => {
    updateActiveConfig({
      providerConfig: {
  ...
  const setActiveConfigName = useCallback((value: string) => {
    updateActiveConfig({ name: value });
  }, [updateActiveConfig]);

  const setConnectionBase = useCallback((value: string) => {
    setBase(value);
    updateActiveConfig({ base: value });
  }, [setBase, updateActiveConfig]);

  const setConnectionApiKey = useCallback((value: string) => {
    setApiKey(value);
    updateActiveConfig({ apiKey: value });
  }, [setApiKey, updateActiveConfig]);

  const setProviderAuthType = useCallback((value: ProviderConfig['authType']) => {
    updateProviderConfig({ authType: value });
  }, [updateProviderConfig]);

  const setProviderModelsEndpoint = useCallback((value: string) => {
    updateProviderConfig({ modelsEndpoint: value });
  }, [updateProviderConfig]);

  const setProviderCustomHeaderName = useCallback((value: string) => {
    updateProviderConfig({ customHeaderName: value });
  }, [updateProviderConfig]);

  const setProviderCustomPrefix = useCallback((value: string) => {
  ...
  const updateProjectModel = useCallback((projectModels: ProjectModel[], modelId: string, patch: Partial<ProjectModel>) => {
    setProjectModels(
      projectModels.map((model) => (
        model.modelId === modelId
          ? { ...model, ...patch, updatedAt: Date.now() }
          : model
      )),
    );
  }, [setProjectModels]);
