import { APP_VERSION } from '@/features/workflow/lib/constants';
    ...
    <div className="workflow-statusbar">
      <div className="workflow-statusbar__frame glass">
        ...
        <div className="workflow-statusbar__message">
          {isExecuting && <span className="workflow-statusbar__tone workflow-statusbar__tone--accent">{executionMessage || '正在执行工作流...'}</span>}
          ...
          {!isExecuting && lastExecutionStatus === 'success' && (
            <span className="workflow-statusbar__tone workflow-statusbar__tone--success">
              运行成功
              {lastExecutionSummary ? ` · ${lastExecutionSummary.successCount} 成功 / ${lastExecutionSummary.failCount} 失败` : ''}
              {lastExecutionTime ? ` · ${formatDurationSeconds(lastExecutionTime)}` : ''}
            </span>
          )}
          {!isExecuting && lastExecutionStatus === 'error' && (
            <span className="workflow-statusbar__tone workflow-statusbar__tone--danger" title={lastExecutionError || undefined}>
              运行失败
              {lastExecutionSummary ? ` · ${lastExecutionSummary.successCount} 成功 / ${lastExecutionSummary.failCount} 失败` : ''}
              {lastExecutionError ? ` · 请检查节点配置或运行日志：${lastExecutionError}` : ''}
            </span>
          )}
