...
import type { ApiConfig, BridgeRef, ModelInfo } from '@/shared/types';
...
import { QUICK_PROMPTS, RATIOS } from '../constants';
        ...
        <div className="flex-col" style={{ gap: 16 }}>
          {sectionCard(
            ...
            <IOSSegmentedControl
              ...
              value={img.mode}
              onChange={(v) => img.setMode(v as 'text' | 'image')}
            />,
          )}
          ...
          {sectionCard(
            ...
            <div className="flex-col" style={{ gap: 12 }}>
              <div>
                ...
                <IOSSelect value={img.model} onChange={img.setModel}>
                  <option value="">选择模型</option>
                  {Object.entries(
                    img.imgModels.reduce<Record<string, ModelInfo[]>>((groups, model) => {
          ...
          {sectionCard(
            ...
            <div className="flex-col" style={{ gap: 12 }}>
              <div style={pairStyle}>
                <div>
                  <IOSLabel>图片比例</IOSLabel>
                  <IOSSelect value={img.ratio} onChange={img.setRatio}>
                    {RATIOS.map((r) => (
                ...
                <div>
                  <IOSLabel>张数</IOSLabel>
                  <IOSSelect value={String(img.count)} onChange={(v) => img.setCount(Number(v) || 1)}>
              ...
              <div style={pairStyle}>
                <div>
                  ...
                  <AutoTextarea
                    value={img.width}
                    onChange={img.setWidth}
                    placeholder="如 1152"
                    maxH={80}
                    onBlur={() => img.normalizeWidth()}
                  />
                </div>
                <div>
                  <IOSLabel>高</IOSLabel>
                  <AutoTextarea
                    value={img.height}
                    onChange={img.setHeight}
                    placeholder="如 2048"
                    maxH={80}
                    onBlur={() => img.normalizeHeight()}
                  />
                </div>
              </div>
              <div style={{ fontSize: 12, color: 'var(--color-text-tertiary)', lineHeight: 1.5 }}>
                {getSizingHint(img.width, img.height, img.ratio)}
              </div>
              <div style={pairStyle}>
                <div>
                  <IOSLabel>输出档位</IOSLabel>
                  <IOSSelect value={img.resolution} onChange={(v) => img.setResolution(v as typeof img.resolution)}>
                ...
                <div>
                  <IOSLabel>输出格式</IOSLabel>
                  <IOSSelect
                    value={img.outputFormat}
                    onChange={(v) => img.setOutputFormat(v as typeof img.outputFormat)}
                  >
          ...
          {img.mode === 'image' &&
            sectionCard(
              ...
              '上传参考图来约束生成方向，最多 5 张。',
              <div>
                <FileUploadArea accept="image/*" multiple onUpload={img.handleFileUpload} />
                <RefImageList
                  images={img.refImages}
                  onRemove={(i) => img.setRefImages((p) => p.filter((_, j) => j !== i))}
                />
              </div>,
            )}
          ...
          {sectionCard(
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {QUICK_PROMPTS.map((q) => (
                <button
                  key={q.label}
                  onClick={() => img.setPrompt(q.prompt)}
                  title={q.prompt}
                  style={{
                    padding: '6px 12px',
                    borderRadius: 12,
          ...
          <IOSButton
            label={`生成图片${img.activeCount > 0 ? ` (${img.activeCount}个任务)` : ''}`}
            onClick={() => {
              img.handleGenerate();
            }}
            color={T.orange}
            disabled={!img.prompt.trim() || !img.model}
          />
        </div>
      </div>
    </div>
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      if (item.url.startsWith('data:')) a.click();
                      else {
                        a.target = '_blank';
                        a.click();
                      }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <WorkbenchInsightCard eyebrow="Run State">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 14 }}>
          {img.tasks.length === 0 && (
            <EmptyPanelState
              title="还没有任务"
              body="这里会记录每一次图片生成的排队、运行和失败状态。"
              action="提交一条图片任务后，可以回到这里查看进度和错误信息。"
            />
          )}
          {img.tasks.map((t) => (
            <div
            ...
            >
              ...
              {t.error && (
                ...
                >
                  <XCircle size={10} /> {t.error.slice(0, 80)}
                </div>
              )}
            </div>
          ))}
      ...
      <WorkbenchInsightCard eyebrow="Snapshot">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 14 }}>
          ...
          <span style={chipStyle(img.mode === 'image' ? T.green : undefined)}>
            {img.mode === 'image' ? '图生图' : '文生图'}
          </span>
    ...
    <>
      ...
      {detailTask && (
        <TaskDetailModal
          ...
          onApply={(t) => {
            if (!('ratio' in t)) return;
            img.setPrompt(t.prompt);
            img.setModel(t.model);
            img.setRatio(t.ratio);
            img.setWidth(t.width ? String(t.width) : '');
            img.setHeight(t.height ? String(t.height) : '');
            img.setQuality(t.quality || 'auto');
            img.setResolution(t.resolution || 'auto');
            img.setCount(t.n || 1);
            img.setOutputFormat(t.output_format || 'png');
            if (t.refImages.length > 0) {
              img.setRefImages(t.refImages);
              img.setMode('image');
            }
          }}
        />
      ...
      <MediaWorkbench
        ...
        toolbarChips={
          <>
            <span style={chipStyle(img.activeCount > 0 ? T.orange : undefined)}>{img.activeCount} 个运行中</span>
            <span style={chipStyle(img.gallery.length > 0 ? T.blue : undefined)}>{img.gallery.length} 张结果</span>
          </>
        }
