...
function AiChatV2Node({ id, data, selected, isConnectable }: AiChatV2NodeProps) {
  const type = 'aiChatV2';
  ...
  const defaultLabel = def?.label || 'AI 对话 V2';
  ...
  return (
    ...
    >
        ...
        <div className="node-v2-appendix-bar">
          {isEditingLabel ? (
            ...
            <span className="node-v2-appendix-label" onClick={startEditing} title="点击重命名">
              <span className="node-v2-appendix-dot" style={{ backgroundColor: def?.color || '#30D158' }} />
      ...
      <NodeResizer isVisible={selected && !isLocked} minWidth={minSize.w} minHeight={minSize.h}
        ...
        lineStyle={{ borderColor: def?.color || '#30D158', borderWidth: 1, borderStyle: 'dashed', opacity: 0.3 }}
        handleStyle={{ width: 14, height: 14, borderRadius: 7, backgroundColor: def?.color || '#30D158', border: '2px solid var(--color-bg-primary)', boxShadow: '0 1px 4px rgba(0,0,0,0.3)' }}
      />

      <Handle type="target" position={Position.Left} id="input" isConnectable={isConnectable}
        style={{ left: -6, top: '50%', transform: 'translateY(-50%)' }} />
      <Handle type="source" position={Position.Right} id="response" isConnectable={isConnectable}
...
export default memo(AiChatV2Node);
