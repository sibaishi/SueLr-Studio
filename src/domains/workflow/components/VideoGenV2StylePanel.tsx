...
type InputThumb = { id: string; type: 'text' | 'image' | 'video' | 'audio'; label: string; value: string };

export default function VideoGenV2StylePanel() {
  ...
  const v2Node = nodes.find((n) => n.selected && n.type === 'videoGenV2');
  ...
  const rawInputs: InputThumb[] = edges
    ...
    .map((e) => {
      ...
      else if (handle.includes('video') || src?.type?.includes('Video')) type = 'video';
      else if (handle.includes('audio') || src?.type?.includes('Audio')) type = 'audio';
  ...
  return (
    ...
    >
        ...
        >
          {inputs.map((input, idx) => (
            ...
            >
              ...
              >
                {(input.type === 'image' || input.type === 'video') && input.value ? (
                  <img
                ...
                {(input.type === 'text' || (!input.value && input.type !== 'audio')) && (
                  <span style={{ fontSize: 9, fontWeight: 700 }}>
                    {input.type === 'text' ? 'T' : input.type === 'image' ? 'IMG' : input.type === 'video' ? 'V' : '?'}
                  </span>
                )}
              </div>
      ...
      <div style={{ flex: 1, minHeight: 0, position: 'relative' }}>
        ...
        >
          <PopoverButton
            icon={<span style={{ fontSize: 12, fontWeight: 700 }}>M</span>}
            label="模型"
          ...
          <PopoverButton
            icon={<Ruler size={14} />}
            label="分辨率"
            open={openPopover === 'resolution'}
            direction="up"
            onToggle={() => setOpenPopover(openPopover === 'resolution' ? null : 'resolution')}
          >
            <select
              value={String(nodeData.resolution || '720p')}
