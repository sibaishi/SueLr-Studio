...
export function createWorkflowGraphEditorActions(
  ...
  return {
    ...
    addEdge: (source, sourceHandle, target, targetHandle) => {
      ...
      const isMultiInput = targetNode?.type === 'imageGenV2' || targetNode?.type === 'videoGenV2' || targetNode?.type === 'aiChatV2' || targetNode?.type === 'aiV3' || targetNode?.type === 'iterateRunV2' || targetNode?.type === 'iterateImageRunV2';
      ...
      if (isMultiInput && targetHandle === 'input') {
        ...
        if (isImageSource) {
          if (targetNode?.type === 'imageGenV2' || targetNode?.type === 'aiChatV2' || targetNode?.type === 'aiV3' || targetNode?.type === 'iterateImageRunV2') {
