...
import { fileToB64 } from '@/shared/runtime/image';
import type { ApiConfig, BridgeRef, ModelInfo, VTask } from '@/shared/types';
import { CHAT_COLOR } from '@/shared/ui/chat-accent';
import {
  AutoTextarea,
  FileUploadArea,
  FullscreenViewer,
...
import { taskStatusColor } from '@/shared/ui/status';
import {
  MediaWorkbench,
  WorkbenchEmptyState,
...
import { VID_DUR, VID_RATIO, VID_RES } from '../constants';
...
export function VideoPanel({
  ...
  onAddToChat: (prompt: string, videoUrl?: string) => void;
  ...
  const resumePollingCard = sectionCard(
    '继续轮询',
    '如果你已经有 taskId，可以手动继续查询这个视频任务。',
    <div className="flex-col" style={{ gap: 12 }}>
      {latestTask && (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}>
            ...
            <span style={chipStyle(taskStatusColor(latestTask.status, T))}>
      ...
      <IOSButton
        ...
        onClick={() => {
          void vid
            .resumeTaskPolling(resumeTaskId)
            .then(() => setResumeTaskId(''))
            .catch((error) =>
              addLog('error', `[Video] 手动继续轮询失败: ${error instanceof Error ? error.message : String(error)}`),
            );
        }}
        color={T.blue}
  ...
  const controlContent = (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0, height: '100%' }}>
      <div style={{ padding: 16, borderBottom: '1px solid var(--color-border)' }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.6, color: 'var(--color-text-secondary)', marginTop: 6 }}>
          设置模型、提示词和素材后提交视频生成任务，最近任务状态和 taskId 也会同步显示在这里。
        </div>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        <div className="flex-col" style={{ gap: 16 }}>
          {sectionCard(
            '状态',
            '视频能力已启用，提交后会显示当前任务状态、taskId 和耗时。',
            <div
              style={{
                padding: '12px 14px',
                borderRadius: 12,
          ...
          {sectionCard(
            ...
            <div className="flex-col" style={{ gap: 12 }}>
              <IOSSegmentedControl
                ...
                value={vid.mode}
                onChange={(v) => vid.setMode(v as 'text' | 'image')}
              />
              <div>
                <IOSLabel>模型</IOSLabel>
                <IOSSelect value={vid.model} onChange={vid.setModel} disabled={videoGenerationDisabled}>
                  <option value="">选择模型</option>
                  {Object.entries(
                    vid.vidModels.reduce<Record<string, ModelInfo[]>>((groups, model) => {
              ...
              <div>
                <IOSLabel>提示词</IOSLabel>
                <AutoTextarea
                  value={vid.prompt}
                  onChange={vid.setPrompt}
          ...
          {sectionCard(
            ...
            <div className="flex-col" style={{ gap: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                <div>
                  <IOSLabel>时长</IOSLabel>
                  <IOSSelect
                    value={String(vid.duration)}
                    onChange={(v) => vid.setDuration(Number(v))}
                    disabled={videoGenerationDisabled}
                  >
                    {VID_DUR.map((d) => (
                ...
                <div>
                  <IOSLabel>分辨率</IOSLabel>
                  <IOSSelect value={vid.resolution} onChange={vid.setResolution} disabled={videoGenerationDisabled}>
                    {VID_RES.map((r) => (
                ...
                <div>
                  <IOSLabel>比例</IOSLabel>
                  <IOSSelect value={vid.vidRatio} onChange={vid.setVidRatio} disabled={videoGenerationDisabled}>
                    {VID_RATIO.map((r) => (
                      <option key={r.v} value={r.v}>
                        {r.l}
                      </option>
              ...
              {vid.mode === 'image' && (
                <div>
                  ...
                  <FileUploadArea
                    accept="image/*"
                    multiple
                    onUpload={vid.handleFileUpload}
                    disabled={videoGenerationDisabled}
                  />
                  <RefImageList
                    images={vid.refImages}
                    onRemove={(i) => vid.setRefImages((p) => p.filter((_, j) => j !== i))}
                  />
                </div>
              )}
              <div>
                ...
                <FileUploadArea
                  ...
                  onUpload={(files) => {
                    const f = files[0];
                    if (f) fileToB64(f).then((b64) => vid.setAudioFile({ name: f.name, type: f.type, data: b64 }));
                  }}
                />
                {vid.audioFile && (
                  <audio controls src={vid.audioFile.data} style={{ width: '100%', height: 32, marginTop: 8 }} />
                )}
              </div>
            </div>,
          )}
          ...
          <IOSButton
            ...
            onClick={vid.submit}
            color={T.purple}
            disabled={!vid.model || !vid.prompt.trim()}
          />
        </div>
      </div>
    </div>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                >
                  <div>
                    ...
                    <div
                      style={{ fontSize: 12, color: 'var(--color-text-primary)', marginTop: 4, wordBreak: 'break-all' }}
                    >
                ...
                <div style={{ fontSize: 11, color: 'var(--color-text-tertiary)', marginTop: 4 }}>
                  创建于：{ftime(latestTask.ts)}
                  {latestTask.updatedAt ? ` · 最近更新：${ftime(latestTask.updatedAt)}` : ''}
                </div>
                {latestTask.error && (
                  <div
                    style={{ fontSize: 12, color: T.red, marginTop: 10, display: 'flex', alignItems: 'center', gap: 6 }}
                  >
              ...
              <div key={v.id} style={{ ...panelStyle(), borderRadius: 18, padding: 12 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    <button
                      onClick={() => onAddToChat(v.prompt, v.url)}
                      style={{
                        padding: '6px 12px',
                        borderRadius: 10,
                        border: 'none',
                        background: `linear-gradient(135deg, ${CHAT_COLOR}, ${T.blue})`,
                        color: '#fff',
                        fontSize: 12,
                        cursor: 'pointer',
                        fontWeight: 700,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <WorkbenchInsightCard eyebrow="Run State">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 14 }}>
          {vid.tasks.length === 0 && (
            <EmptyPanelState
              title="还没有视频任务"
              body="这里会展示视频任务的提交、轮询、完成和失败状态。"
              action="提交任务后，可以回到这里查看进度和 taskId。"
            />
          )}
          {vid.tasks.map((t) => (
            <div
              ...
              style={{ ...mutedPanelStyle(), padding: 14, cursor: 'pointer' }}
            >
              ...
              {t.error && (
                <div
                  style={{ fontSize: 11, color: T.red, marginTop: 6, display: 'flex', alignItems: 'center', gap: 4 }}
                >
                  <XCircle size={10} /> {t.error.slice(0, 120)}
                </div>
              )}
            </div>
          ))}
      ...
      <WorkbenchInsightCard eyebrow="Snapshot">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 14 }}>
          ...
          <span style={chipStyle(vid.mode === 'image' ? T.green : undefined)}>
            {vid.mode === 'image' ? '图生视频' : '文生视频'}
          </span>
          <span style={chipStyle(vid.activeCount > 0 ? T.orange : undefined)}>{vid.activeCount} 个运行中</span>
          <span style={chipStyle(vid.completedVideos.length > 0 ? T.blue : undefined)}>
            {vid.completedVideos.length} 个结果
          </span>
        </div>
      </WorkbenchInsightCard>
    </div>
    ...
    <>
      ...
      {detailTask && (
        <TaskDetailModal
          ...
          onApply={(t) => {
            vid.setPrompt(t.prompt);
            vid.setModel(t.model);
          }}
        />
      )}
      <MediaWorkbench
        ...
        toolbarMeta={
          videoGenerationDisabled
            ? '视频生成暂时停用'
            : latestTask
              ? `${getVideoTaskStatusLabel(latestTask.status)}${latestTask.taskId ? ` · ${latestTask.taskId}` : ''}`
              : vid.model || '还没有选择模型'
        }
        toolbarChips={
          <>
            <span style={chipStyle(T.purple)}>已启用</span>
            <span style={chipStyle(vid.activeCount > 0 ? T.orange : undefined)}>{vid.activeCount} 个运行中</span>
            <span style={chipStyle(vid.completedVideos.length > 0 ? T.blue : undefined)}>
              {vid.completedVideos.length} 个结果
            </span>
