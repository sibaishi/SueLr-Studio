...
const WELCOME_SUGGESTIONS = ['帮我写一篇文章', '解释一段代码', '整理一份会议纪要', '帮我规划一个项目'];
...
function ContextUsagePanel({
  ...
  return (
    <div style={{ ...mutedPanelStyle(), padding: 14, overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 23, fontWeight: 800, color: stats.status.color }}>
              {stats.totalTokens.toLocaleString()}
            </span>
            <span style={{ fontSize: 11, color: 'var(--color-text-tertiary)' }}>
              / {stats.contextWindow.toLocaleString()} tokens 阈值
            </span>
          </div>
          ...
          >
            <div
              style={{
                height: '100%',
                width: `${stats.usagePct}%`,
                borderRadius: 999,
                background: `linear-gradient(90deg, ${T.green}, ${stats.status.color})`,
                transition: 'width 0.3s ease',
              }}
            />
          </div>
          <div style={{ fontSize: 11, lineHeight: 1.5, color: 'var(--color-text-tertiary)', marginTop: 8 }}>
            {stats.status.hint}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 14 }}>
        {stats.breakdown.map((item) => (
          ...
          >
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--color-text-primary)', marginTop: 6 }}>
      ...
      >
        <span>
          {stats.messageCount} 条消息 · {stats.totalChars.toLocaleString()} 字符
        </span>
        {stats.compressed && (
...
export function ChatPanel({
  ...
  const modelOptions = chat.chatModels.map((model) => ({
  ...
  const contextUsage = useMemo(() => {
    const totalChars = activeConversation.msgs.reduce(
    ...
    const pendingFileTokens = estimateTokensFromText(
      chat.pendingFiles.map((file) => `${file.name}\n${file.content}`).join('\n\n'),
    );
    ...
    const compressed = activeConversation.msgs.some(
      (msg) =>
        msg.role === 'assistant' &&
    ...
    chat.currentRole.name,
    chat.currentRole.systemPrompt,
    chat.pendingFiles,
    chat.pendingImages.length,
    chat.tokenUsage,
  ]);
  const conversations = (
  ...
  return (
    ...
    >
        ...
        <div className="workflow-toolbar__frame" style={{ alignItems: 'stretch', flexWrap: 'wrap', rowGap: 12 }}>
          ...
          <div className="workflow-toolbar__status" style={{ minWidth: 260, flex: 1, justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              ...
              <span style={chipStyle(chat.webSearchEnabled ? T.purple : undefined)}>
                {chat.webSearchEnabled ? '联网搜索已开' : '联网搜索已关'}
              </span>
            </div>
          </div>
          ...
          {chat.sendings.has(chat.conv.id) && (
            <div
              className="workflow-toolbar__group workflow-toolbar__group--actions"
              style={{ marginLeft: 'auto', flexWrap: 'wrap' }}
            >
              {chat.sendings.has(chat.conv.id) && (
                <button
                  onClick={() => chat.cancel(chat.conv.id)}
                  style={{
                    padding: '8px 14px',
                    borderRadius: 12,
      ...
      >
        <aside style={{ ...panelStyle(), overflow: 'hidden', minWidth: 0 }}>{conversations}</aside>
            ...
            <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background:
                    activeConversation.msgs.length > 0 && !chatBusy ? `${T.purple}14` : 'var(--color-bg-secondary)',
            ...
            {chat.conv.msgs.length === 0 && (
              ...
              >
                ...
                <div style={{ fontSize: 13, lineHeight: 1.6, color: 'var(--color-text-secondary)' }}>
                  输入问题、粘贴素材，或直接使用下面的快捷提示词开始。
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
                  {WELCOME_SUGGESTIONS.map((text) => (
                    <button
                      key={text}
                      onClick={() => chat.setInput(text)}
                      style={{
                        padding: '10px 14px',
                        borderRadius: 14,
                        border: '1px solid var(--color-border)',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginBottom: 8 }}>
                    <span
                      style={{
                        fontSize: 10,
                        color: msg.role === 'user' ? 'rgba(255,255,255,0.72)' : 'var(--color-text-tertiary)',
                      }}
                    >
                      {msg.role === 'user' ? '你' : activeRole?.name || 'AI'}
                    </span>
                    <span style={{ display: 'flex', gap: 4 }}>
                      ...
                      {msg.role === 'assistant' && (
                        <button
                          onClick={() => chat.regenerate(msg.id)}
                          style={{ border: 'none', background: 'transparent', color: 'inherit', cursor: 'pointer' }}
                        >
                          <RefreshCw size={11} />
                        </button>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
                      {msg.images.map((img, index) => (
                        <img
                          key={index}
                          src={img}
                          onClick={() => chat.setPreviewUrl(img)}
                          style={{
                            maxWidth: 260,
                            maxHeight: 280,
                            borderRadius: 14,
                    ...
                    >
                      ...
                      {(msg.toolCall.runId || msg.toolCall.detail || msg.toolCall.error) && (
                        <span style={{ fontSize: 11, lineHeight: 1.5, color: 'var(--color-text-secondary)' }}>
                          {msg.toolCall.error ||
                            msg.toolCall.detail ||
                            (msg.toolCall.runId ? `runId: ${msg.toolCall.runId}` : '')}
                        </span>
                      )}
                      ...
                      {Array.isArray(msg.toolCall.artifacts) && msg.toolCall.artifacts.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4, width: '100%' }}>
                          <div style={{ fontSize: 11, color: 'var(--color-text-secondary)' }}>
                            {msg.toolCall.type === 'workflow' ? '工作流输出' : '工具输出'}
                          </div>
          ...
          <footer style={{ padding: 16, borderTop: '1px solid var(--color-border)', background: 'var(--glass-bg)' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                ...
                {chat.pendingFiles.map((file) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => chat.removePendingFile(file.id)}
                      style={{
                        border: 'none',
                        background: 'transparent',
                        color: 'var(--color-text-tertiary)',
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
              <input
                ...
                type="file"
                accept="image/*,.txt,.md,.markdown,.csv,.json,.xml,.html,.css,.js,.jsx,.ts,.tsx,.py,.java,.go,.rs,.c,.cpp,.h,.hpp,.yaml,.yml,.toml,.ini,.log"
                multiple
                onChange={(event) => {
                  if (event.target.files) chat.handleFileUpload(event.target.files);
                  event.target.value = '';
                }}
                style={{ display: 'none' }}
              />
          ...
          <div style={{ ...panelStyle(), padding: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 14 }}>
              ...
              <div style={{ ...mutedPanelStyle(), padding: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
                  <span style={chipStyle(chat.currentModel ? T.green : undefined)}>
                    {chat.currentModel ? '模型可用' : '还没有选择模型'}
                  </span>
